...
class TextFormatter {
  ...
  private static String finalText2;
	 ...
	 * StringTokenizer toki = new StringTokenizer(text); String empty = "";
	 * while(toki.hasMoreTokens()){ if (empty.length() <= maxLineLength) { empty
	 * += toki.nextToken() + ' '; //System.out.println(empty); }else{ //empty +=
	 * "\n"; System.out.println(empty); empty = ""; //System.out.println(empty);
		...
		while (toki.hasMoreTokens()) {
			...
			if (empty.length() >= (maxLineLength - word.length())) {
			  zeile = empty;
				...
				spaces = (maxLineLength - empty.length());
				while (spaces != 0) {
				  zeile = " " + zeile;
					spaces--;
				}
